-- ============================================================
-- Repair for the social-feed helper functions.
--
-- Symptom: feed posts showed the author as "Someone" and shared workouts as
-- "Could not load workout details." Both are fetched via RPC (feed_author_profiles
-- and feed_workout_summary) and the client swallows errors, so a missing or
-- mismatched function surfaces exactly this way.
--
-- This drops any prior versions (clearing a bad/partial apply) and recreates
-- both. Safe and idempotent — run anytime.
-- ============================================================

drop function if exists public.feed_author_profiles(uuid[]);
drop function if exists public.feed_workout_summary(uuid);

-- Basic public profile info for a set of users (names/avatars are public within
-- the app's social feed). profiles is owner-only under RLS, so a direct select
-- can't read other users' rows — hence this SECURITY DEFINER function.
create function public.feed_author_profiles(p_ids uuid[])
...
create function public.feed_workout_summary(p_session_id uuid)
returns jsonb
language sql
stable
security definer
