 ...
 *   EMAIL_FROM       sender, default "Ares Fitness <onboarding@resend.dev>"
...
const EMAIL_FROM = process.env.EMAIL_FROM ?? "Ares Fitness <onboarding@resend.dev>";
...
export async function notifyAdminNewMember(member: {
  ...
  <div style="max-width:520px;margin:0 auto;background:#1A1A1A;border:1px solid rgba(255,255,255,.08);border-radius:20px;overflow:hidden">
    <div style="padding:20px 24px;border-bottom:1px solid rgba(255,255,255,.08)">
      <span style="font-size:18px;font-weight:800">Ares <span style="color:#FF520E">Fitness</span></span>
    </div>
    <div style="padding:24px">
      <p style="margin:0 0 4px;color:#FF520E;font-size:12px;letter-spacing:.06em;text-transform:uppercase">New member</p>
  ...
  <p style="max-width:520px;margin:12px auto 0;color:#737373;font-size:12px;text-align:center">Automated notification from Ares Fitness.</p>
  </body></html>`;

  const text = `New member joined Ares Fitness\n\n${rows
    .map(([k, v]) => `${k}: ${v}`)
    .join("\n")}`;

  return sendEmail({
    to: ADMIN_EMAIL,
    subject: `New Ares Fitness member: ${name}`,
    html,
    text,
...
export async function notifyAdminNewFeedback(input: {
  ...
  <div style="max-width:520px;margin:0 auto;background:#1A1A1A;border:1px solid rgba(255,255,255,.08);border-radius:20px;overflow:hidden">
    ...
    <div style="padding:24px">
      <p style="margin:0 0 4px;color:#FF520E;font-size:12px;letter-spacing:.06em;text-transform:uppercase">${escapeHtml(label)}</p>
  ...
  <p style="max-width:520px;margin:12px auto 0;color:#737373;font-size:12px;text-align:center">Automated notification from Ares Fitness. Reply to respond to the member.</p>
  </body></html>`;
  ...
  return sendEmail({
    to: ADMIN_EMAIL,
    subject: `Ares Fitness ${label.toLowerCase()} from ${who}`,
    html,
    text,
