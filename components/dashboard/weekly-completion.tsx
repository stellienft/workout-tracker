...
export function WeeklyCompletionCard({
  ...
  // Show required workouts (weekly split / calendar) or the rotation, in order.
  const required = [...templates]
    .filter((t) => !t.is_optional)
    .sort(
      (a, b) =>
        (a.sequence_order ?? a.week_position ?? 0) -
        (b.sequence_order ?? b.week_position ?? 0)
    )
    .slice(0, 6);
  ...
  return (
    <div className="h-full rounded-[var(--radius-card)] border border-[var(--border-subtle)] bg-[var(--surface-primary)] p-5">
      ...
      <div className="mt-3 flex items-center gap-4">
        <ProgressRing percent={progress.percent} size={72} />
        <div>
          <p className="text-lg font-bold">
            {progress.completed} of {progress.target} workouts
          </p>
