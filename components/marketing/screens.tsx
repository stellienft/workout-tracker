    ...
    <div className={`relative ${className}`}>
      ...
      >
        ...
        <div className="absolute inset-2.5 flex flex-col overflow-hidden rounded-[36px] bg-bg">
          ...
          <div className={`min-h-0 flex-1 overflow-hidden px-4 pt-3 ${nav ? "pb-[60px]" : ""}`}>{children}</div>
...
function BottomNav({ active }: { active?: Tab }) {
  return (
    <div className="absolute inset-x-0 bottom-0 z-10 flex items-center justify-around border-t border-white/8 bg-[#111]/95 px-2 pb-3 pt-2.5 backdrop-blur">
      {TABS.map((t) => {
